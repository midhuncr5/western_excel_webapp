...
# --------------------------
@st.cache_data(ttl=60)
def download_excel_as_df(file_id):
    request = drive_service.files().get_media(fileId=file_id)
    fh = io.BytesIO()
    downloader = MediaIoBaseDownload(fh, request)
    done = False
...
# UPLOAD EXCEL
# --------------------------
def upload_excel_from_df(file_id, df):
    out = io.BytesIO()
    df.to_excel(out, index=False, engine="openpyxl")
    out.seek(0)
    media = MediaIoBaseUpload(
...
# --------------------------
# REMOVE FULLY REJECTED ROWS (VIEW ONLY)
# --------------------------
df = df[
    ~(
...
# NUMERIC FIXES
# --------------------------
for c in ["BASIC_AMOUNT", "ADJUSTMENT_AMOUNT"]:
    df_display[c] = pd.to_numeric(df_display[c], errors="coerce").fillna(0)

mask = (
    df_display["STATUS_MATCHED_ESTIMATION"]
    .astype(str).str.upper().eq("ESTIMATION NOT MATCHED")
    & (df_display["BASIC_AMOUNT"] != 0)
    & (df_display["ADJUSTMENT_AMOUNT"] == 0)
)
...
with st.form("edit_form"):
    edited_df = st.data_editor(
        df_display,
        use_container_width=True,
        hide_index=False,      # KEEP INDEX
        num_rows="fixed",     # NO ROW ADD/DELETE
        column_config={
...
if submit:
    try:
        df.loc[edited_df.index, "APPROVAL_1"] = edited_df["APPROVAL_1"]
        df.loc[edited_df.index, "APPROVAL_2"] = edited_df["APPROVAL_2"]
        ...
        st.success("✅ Excel saved correctly without corruption")
...
if search:
    view_df = df_display[
        df_display.apply(
            ...
            .str.contains(search, case=False).any(),
            axis=1
        )
    ]
else:
...
st.info("ℹ️ Excel overwrite is safe. Only approval columns are updated.")
