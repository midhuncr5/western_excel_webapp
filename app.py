...
df_display = df_original[
    ~(
        (df_original["APPROVAL_1"].astype(str).str.upper() == "REJECTED") &
        (df_original["APPROVAL_2"].astype(str).str.upper() == "REJECTED")
    )
].copy()
...
df_display = df_display[DISPLAY_COLUMN_ORDER]

# Numeric cleanup
for col in ["BASIC_AMOUNT", "ADJUSTMENT_AMOUNT"]:
    df_display[col] = pd.to_numeric(df_display[col], errors="coerce").fillna(0)

# Auto-fill adjustment amount
mask = (
    df_display["STATUS_MATCHED_ESTIMATION"].astype(str).str.upper() == "ESTIMATION NOT MATCHED"
) & (df_display["BASIC_AMOUNT"] != 0) & (df_display["ADJUSTMENT_AMOUNT"] == 0)
...
if st.button("💾 Save Changes to Drive"):
    try:
        df_original.loc[df_display.index, DISPLAY_COLUMN_ORDER] = edited_df
        upload_excel_from_df(FILE_ID, df_original)

        # Reload fresh data
        st.session_state.df = download_excel_as_df(FILE_ID)

        st.success("✅ Saved successfully and refreshed from Drive")
...
st.info("ℹ️ Data is saved directly to Google Drive and refreshed immediately.")
